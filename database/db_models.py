...
class User(SQLModel, table=True):
    ...
    user_memory: Optional[dict] = Field(default_factory=dict, sa_type=JSON, description="유저의 추가 정보(JSON 형식)")
...
class Message(SQLModel, table=True):
    ...
    conversation_id: int = Field(foreign_key="conversation.id", description="연결된 대화 ID")
    sender: str = Field(description="보낸 사람 (user 또는 bot)")
    message_text: str = Field(description="메시지 내용")
    message_type: Optional[str] = Field(default="text", description="메시지 유형 (예: text, image 등)")
    created: Optional[datetime] = Field(default=None, description="생성 시각")
    updated: Optional[datetime] = Field(default=None, description="업데이트 시각")